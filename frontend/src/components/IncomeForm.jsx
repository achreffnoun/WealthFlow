...
export default function IncomeForm({ initial, onSubmit, onCancel }) {
  ...
  const [source, setSource] = useState(initial?.source ?? INCOME_SOURCES[0].code)
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true)
    try {
      await onSubmit({ amount: numericAmount, source, date, description })
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <label className="block font-label-sm text-on-surface-variant uppercase tracking-wider mb-2">Source</label>
        <select
          value={source}
      ...
      <div>
        ...
        <input
          ...
          placeholder="Ex: Salaire juillet"
          className="w-full rounded-lg border border-outline-variant bg-surface-container-low focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none text-body-md p-3"
        />
      </div>
